...
def Hammutabi():
    ...
    bushels_per_acre = 3000  # amount harvested for each acre planted
    rats_ate = 200
...
There were''', plague_deaths, '''from the plague'''
        ...
        num_feed = ask_to_feed()

        bushels_in_storage -= num_feed

        num_cultivate = ask_to_cultivate()
        ...
        immigrants = numImmigrants(acres_owned, bushels_in_storage, starved)

        population += immigrants

        bushels_in_storage += num_cultivate * getHarvest() * doRatsInfest
